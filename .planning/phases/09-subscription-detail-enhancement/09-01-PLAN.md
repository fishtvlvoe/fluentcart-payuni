...
must_haves:
  truths:
    - "Merchant can view renewal history (date, amount, status, retry count) in subscription detail page"
    - "Merchant can see bound payment card (last 4 digits, expiry, brand) in subscription detail page"
    - "Merchant can see failure reason and retry status when renewal fails"
    - "Merchant can see next billing date and expected renewal amount"
  artifacts:
  ...
  key_links:
    ...
    - from: "fluentcart-payuni.php"
      ...
      via: "class instantiation with class_exists guard"
      pattern: "class_exists.*SubscriptionPayUNiMetaBox.*new"
---
...
# FluentCart subscription model documentation (VERIFIED API reference)
...
**IMPORTANT - FluentCart API Verification:**
The following APIs are documented in `docs/fluentcart-reference/fluentcart.com_doc/database_models_subscription.md`:
- `$subscription->transactions` (line 513-523): Returns Collection of OrderTransaction
- `$subscription->getMeta($key, $default)` (line 201-212): Get subscription meta value
- `$subscription->getCurrentRenewalAmount()` (line 364-374): Get current renewal amount in cents

Load subscription model with defensive class_exists check:
```php
...
1. **renewal_history** (array of renewal transactions):
   - Query transactions via `$subscriptionModel->transactions` relation (verified in docs line 513-523)
...
2. **card_info** (bound payment card):
   - Get from subscription meta `active_payment_method` using getMeta() (verified in docs line 201-212):
     ```php
...
3. **failure_info** (when subscription is failing):
   ...
   - Translate error messages: missing_credit_hash -> "Token", requires_3d -> " 3D ", etc.
...
4. **next_billing_info** (enhanced billing date display):
   ...
   - `expected_amount`: from `$subscriptionModel->getCurrentRenewalAmount()` (verified in docs line 364-374), formatted as currency
   - `billing_interval`: subscription billing_interval
   - `billing_interval_label`: human-readable (monthly -> "", yearly -> "")
...
# Verify FluentCart API usage patterns (defensive checks)
grep -E "class_exists.*Subscription|getMeta|getCurrentRenewalAmount|->transactions" /Users/fishtv/Development/fluentcart-payuni/src/Admin/SubscriptionPayUNiMetaBox.php
```
  </verify>
  <done>
...
- [x] Defensive class_exists check for FluentCart Subscription model
- [x] renewal_history extracted from subscription transactions relation
- [x] card_info extracted from active_payment_method meta using getMeta()
- [x] failure_info extracted from payuni_last_error and payuni_renewal_retry meta
- [x] next_billing_info includes expected amount from getCurrentRenewalAmount()
...
1. **Add SubscriptionPayUNiMetaBox instantiation** in the bootstrap function, after existing admin class instantiations. Find the section where OrderPayUNiMetaBox is instantiated (search for `new.*OrderPayUNiMetaBox`) and add nearby:
```php
...
2. **Update existing inline filter**: Find the existing `fluent_cart/subscription/view` filter (search for `add_filter.*fluent_cart/subscription/view`). This filter handles:
...
Check integration:
```bash
# Check SubscriptionPayUNiMetaBox is instantiated with class_exists guard
grep -E "class_exists.*SubscriptionPayUNiMetaBox.*new|new.*SubscriptionPayUNiMetaBox" /Users/fishtv/Development/fluentcart-payuni/fluentcart-payuni.php

# Verify the pattern: class_exists check followed by instantiation
grep -A 2 "class_exists.*SubscriptionPayUNiMetaBox" /Users/fishtv/Development/fluentcart-payuni/fluentcart-payuni.php
...
```
  ...
  <done>
- [x] SubscriptionPayUNiMetaBox instantiated in bootstrap function with class_exists guard
...
# Verify bootstrap integration pattern (class_exists -> new)
grep -A 2 "class_exists.*SubscriptionPayUNiMetaBox" /Users/fishtv/Development/fluentcart-payuni/fluentcart-payuni.php
```
...
- [ ] Defensive class_exists check for FluentCart Subscription model
...
- [ ] Bootstrap instantiates SubscriptionPayUNiMetaBox with class_exists guard
