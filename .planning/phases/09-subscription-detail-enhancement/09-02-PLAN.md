...
must_haves:
  truths:
    - "Subscription detail page displays renewal history table (date, amount, status, retry count)"
    - "Subscription detail page shows bound payment card with masked number and expiry"
    - "Subscription detail page shows failure reason when renewal fails"
    - "Subscription detail page shows next billing date and expected amount"
  artifacts:
...
After creating the existing buttons wrap (around line 120), add:
...
**Important placement:** Add this code AFTER the existing nextBillingRow section (around line 167) and BEFORE the `container.appendChild(body);` line.
...
Find the existing section that enqueues `payuni-subscription-detail.js` (around line 540-560) in the `add_action('admin_enqueue_scripts', ...)` for FluentCart admin pages.

**Add CSS enqueue** before the JS enqueue:
...
This should be added in the same `admin_enqueue_scripts` callback that already loads `payuni-subscription-detail.js`, typically within the `if (strpos($hook, 'page=fluent-cart') !== false)` condition.
