...
<task type="checkpoint:verify">
  <name>Task 3: 驗證 Processor 的 MerTradeNo 符合規範</name>
  <what-built>IdempotencyService 和 PayUNiAPI 的 idempotency key 機制</what-built>
  <how-to-verify>
驗證現有 Processor 中的 MerTradeNo 產生邏輯：

1. **檢查 PaymentProcessor.php**：
   ```bash
   grep -n "MerTradeNo" src/Processor/PaymentProcessor.php
   ```
   - 確認 MerTradeNo 格式符合 PayUNi 20 字元限制
   - 如果超過 20 字元，記錄為待修正問題

2. **檢查 SubscriptionPaymentProcessor.php**：
   ```bash
   grep -n "MerTradeNo" src/Processor/SubscriptionPaymentProcessor.php
   ```
   - 同上

3. **驗證唯一性**：
   - 確認每次呼叫產生不同的 MerTradeNo
   - 可在測試環境執行兩次付款流程，檢查 log 中的 MerTradeNo 是否不同
...
- 如果現有格式已符合，無需修改程式碼
  ...
  <resume-signal>
確認結果：
- 符合規範：輸入 "verified - MerTradeNo 已符合規範"
- 需要修改：輸入 "needs-fix - 格式為 {現有格式}，超過 20 字元"
  </resume-signal>
</task>
