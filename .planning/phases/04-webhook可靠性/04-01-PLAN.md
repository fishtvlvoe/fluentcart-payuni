...
3. **確保 autoloader 可以載入 `FluentcartPayuni\Database`**：
   - 檢查 `composer.json` 的 `autoload.classmap` 或 `autoload.psr-4` 設定
   - 如果 `includes/` 尚未在 autoload 設定中，新增：
     ```json
     "autoload": {
         "classmap": ["includes/"]
     }
     ```
   - 執行 `composer dump-autoload` 更新 autoloader
   - 或者，如果專案不使用 Composer autoload，在 `fluentcart-payuni.php` 中使用 `require_once`（已在 hook 中處理）

位置：在現有的 `register_activation_hook` 之後或替換現有的啟用邏輯。
  </action>
  <verify>
1. 執行 `composer dump-autoload`（如果使用 Composer autoload）
2. 停用並重新啟用外掛
3. 執行 `SHOW TABLES LIKE '%payuni_webhook_log%'` 確認資料表存在
4. 執行 `DESCRIBE {prefix}payuni_webhook_log` 確認欄位正確
5. 驗證 autoload 可載入：`php -r "require 'vendor/autoload.php'; new FluentcartPayuni\Database();"`
  </verify>
  <done>
